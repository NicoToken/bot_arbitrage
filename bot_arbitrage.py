...
# Fungsi untuk menghitung threshold berdasarkan histori harga

def calculate_threshold(price_history):

    mean_price = sum(price_history) / len(price_history)

    std_dev = sum([(price - mean_price) ** 2 for price in price_history]) / len(price_history)
...
def run_arbitrage(symbol, threshold, quantity, target_profit):
    ...
    while True:
        ...
        if len(price_history) > 5:
            ...
            calculated_threshold = calculate_threshold(price_history)

            if price_diff > calculated_threshold:
...
# Meminta input dari pengguna untuk simbol, threshold, kuantitas, dan target keuntungan
